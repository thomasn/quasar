// $Id: card_inquiry.cpp,v 1.29 2005/01/30 04:25:31 bpepers Exp $
//
...
CardInquiry::CardInquiry(MainWindow* main, QWidget* parent)
    : InquiryWindow(main, parent, "CardInquiry", new CardLookup(main, parent))
{
    search->setLength(30);
}
...
void
CardInquiry::slotRefresh(QDate from, QDate to, Id store_id)
{
    if (loading) return;
    loading = true;
    needsRefresh = false;
    ...
    // Get card to show and return if none
    Id card_id = search->getId();
    ...
    if (!from.isNull())
	beginBalance = quasar->db()->cardBalance(card_id, from - 1, store_id);

    // Select transactions
    vector<Gltx> gltxs;
    vector<fixed> amounts;
    quasar->db()->selectCard(card_id, store_id, from, to, gltxs, amounts);

    // Setup grid
    _grid = new Grid(8);
    Grid* header = new Grid(1, _grid);
    TextFrame* text;
    text = new TextFrame(tr("Card Inquiry"), header);
	...
	// Don't show quotes
	if (gltx.dataType() == DataObject::QUOTE)
	    continue;

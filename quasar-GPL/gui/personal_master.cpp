// $Id: personal_master.cpp,v 1.28 2005/02/03 08:19:28 bpepers Exp $
//
...
PersonalMaster::PersonalMaster(MainWindow* main, Id personal_id)
    : DataWindow(main, "PersonalMaster", personal_id)
{
    _helpSource = "personal_master.html";
    ...
    ExtraLookup* extraLookup = new ExtraLookup(_main, this, "Personal");
    ...
    grid->addMultiCellWidget(addr, 3, 3, 0, 2);
    grid->addMultiCellWidget(tabs, 4, 4, 0, 2);

    setCaption(tr("Personal Master"));
    finalize();
}
...
void
PersonalMaster::cloneFrom(Id id)
{
...
bool
PersonalMaster::deleteItem()
{
    return _quasar->db()->remove(_curr);
}

void
PersonalMaster::restoreItem()
{
    _curr = _orig;
}

void
PersonalMaster::cloneItem()
{
    PersonalMaster* clone = new PersonalMaster(_main);
    clone->cloneFrom(_curr.id());
    clone->show();
}

bool
PersonalMaster::isChanged()
{
    return _curr != _orig;
}
...
void
PersonalMaster::widgetToData()
{
    //_curr.setType(Personal::Personal);
...
void
PersonalMaster::companyChanged(bool company)
{
    if (company) {
...
void
PersonalMaster::groupCellChanged(int row, int col, Variant)
{
...
void
PersonalMaster::extraFocusNext(bool&, int& newRow, int& newCol, int type)
{

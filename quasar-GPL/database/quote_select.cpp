// $Id: quote_select.cpp,v 1.3 2004/01/30 23:16:36 arandell Exp $
//
...
QString
QuoteSelect::where() const
{
    QString where = Select::where();

    if (id != INVALID_ID)
	addIdCondition(where, "quote_id", id);
    if (number != "")
	addStringCondition(where, "number", number);
    if (customer_id != INVALID_ID)
	addIdCondition(where, "customer_id", customer_id);

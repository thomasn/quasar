// $Id: card.cpp,v 1.23 2005/02/03 08:18:03 bpepers Exp $
//
...
Card::Card()
    : _company(false), _first_name(""), _last_name(""), _number(""),
      _street(""), _street2(""), _city(""), _province(""), _country(""),
      _postal(""), _phone_num(""), _phone2_num(""), _fax_num(""),
      _email(""), _web_page(""), _contact(""), _comments("")
{
}
...
Card::operator==(const Card& rhs) const
{
    ...
    if (rhs._company != _company) return false;
    if (rhs._first_name != _first_name) return false;
    if (rhs._last_name != _last_name) return false;
    ...
    if (rhs._comments != _comments) return false;
    if (rhs._patgroup_id != _patgroup_id) return false;
...
QString
Card::address(bool includeExtra) const
{
    QString text = nameFL();
    if (_street != "") {
	if (text != "") text += "\n";
